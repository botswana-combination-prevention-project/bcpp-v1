...
def get_new_transactions(request, **kwargs):
    ...
    if producer:
        ...
        url = '%s/bhp_sync/api/transaction/?format=json&producer=%s&username=%s&api_key=%s' % (producer.url, producer.name, request.user.username, request.user.api_key.key)
        url = url.replace('//', '/')
        ...
        while next_url:
            ...
            if not err:
                ...
                try:
                    ...
                    if next_url:
                        messages.add_message(request, messages.INFO, 'Fetching next %s starting at %s of %s' % (json_response['limit'], json_response['offset'], json_response['total_count']) )                              
                except:
