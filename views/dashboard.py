...
def dashboard(request, **kwargs):
    ...
    if kwargs.get('dashboard_type') == 'subject':
        if kwargs.get('household_member'):
            ...
            if RegisteredSubject.objects.filter(registration_identifier=household_member.internal_identifier).exists():
                # this is the first hsm entry so registration_identifier=hsm.pk
                registered_subject = RegisteredSubject.objects.get(registration_identifier=household_member.internal_identifier)
                subject_identifier = registered_subject.registration_identifier
            else:
                raise ValueError('bcpp_survey.views.dashboard expects all household_members to have an entry in RegisterSubject. Got None for %s.' % (household_member,))
        elif kwargs.get('pk'):
            household_member = HouseholdMember.objects.get(pk=kwargs.get('pk'))
            if RegisteredSubject.objects.filter(registration_identifier=household_member.internal_identifier).exists():
                # this is the first hsm entry so registration_identifier=hsm.pk
                registered_subject = RegisteredSubject.objects.get(registration_identifier=household_member.internal_identifier)
                subject_identifier = registered_subject.registration_identifier
            ...
            subject_identifier = registered_subject.registration_identifier
            household_member = HouseholdMember.objects.get(internal_identifier=registered_subject.registration_identifier, survey__survey_slug=survey.survey_slug)
            ...
            survey = household_member.survey.survey_slug
            registered_subject = RegisteredSubject.objects.get(registration_identifier=household_member.internal_identifier)
            subject_identifier = registered_subject.registration_identifier
            ...
            household_member = HouseholdMember.objects.get(registered_subject=registered_subject)
            #household_member = HouseholdMember.objects.get(internal_identifier=registered_subject.registration_identifier)
            #survey = household_member.survey.survey_slug
            ...
            household_member = None
            #household_member = HouseholdMember.objects.get(internal_identifier=registration_identifier)
            #survey = household_member.survey.survey_slug
            subject_identifier = registered_subject.subject_identifier
            if not subject_identifier:
                subject_identifier = registered_subject.registration_identifier
        else:
            raise ValueError('dashboard view needs a value for attribute \'household_member\'.')
        if not household_member:
            raise ValueError('dashboard view needs a value for attribute \'household_member\'.')
        dashboard = SubjectDashboard(**kwargs)
        # create a basic dashboard context
        kwargs.update({'registered_subject': registered_subject})
        kwargs.update({'household_member': household_member})
        kwargs.update({'subject_consent': household_member.consent()})
        kwargs.update({'extra_url_context': '&household_member={household_member}&survey={survey}'.format(household_member=household_member.pk,
                                                                                                                              survey=household_member.survey.survey_slug,)})
