...
class HouseholdHeadEligibilityForm(BaseModelForm):
    ...
    def clean(self):
        ...
        if HouseholdHeadEligibility.objects.filter(
                household_structure=cleaned_data.get('household_structure', None),
                aged_over_18='Yes', verbal_script='Yes').exists():
            raise forms.ValidationError('You have already entered an eligible head '
                                        'of household. DO NOT enter another one. Continue with Survey.')
