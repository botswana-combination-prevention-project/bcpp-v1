...
#             with pyodbc.connect(settings.LAB_IMPORT_DMIS_DATA_SOURCE) as cnxn:
#                 with cnxn.cursor() as self.cursor:
#                     for selected_column, edc_specimen_identifier, lis_specimen_identifier, lis_patient_ref, received_datetime in fetchall():
...
#                         dmis_column=self.dmis_column, search_list='\',\''.join(self.results.get('received_items_by_specimen_identifier').keys()))
#                 )
#             return self.cursor.execute(self.sql.get('resulted_items')).fetchall()
#         if self.verbose:
#             print "Querying dmis for {} resulted items".format(len(self.results.get('received_items_by_specimen_identifier').keys()))
#         try:
#             with pyodbc.connect(settings.LAB_IMPORT_DMIS_DATA_SOURCE) as cnxn:
#                 with cnxn.cursor() as self.cursor:
...
#                         received_tuple = self.results.get('received_items_by_specimen_identifier').get(lis_specimen_identifier)
#                         try:
