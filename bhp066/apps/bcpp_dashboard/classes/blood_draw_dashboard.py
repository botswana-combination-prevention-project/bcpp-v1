...
from apps.bcpp_subject.models import SubjectConsentRbd, SubjectReferral
...
class BloodDrawDashboard(BaseSubjectDashboard):
    ...
    def __init__(self, **kwargs):
        ...
        kwargs.update({'dashboard_models': {'subject_consent': SubjectConsentRbd}})
    ...
    def consent(self):
        ...
        self._consent = None
        if SubjectConsentRbd.objects.filter(subject_identifier=self.subject_identifier):
            self._consent = SubjectConsentRbd.objects.get(subject_identifier=self.subject_identifier)
