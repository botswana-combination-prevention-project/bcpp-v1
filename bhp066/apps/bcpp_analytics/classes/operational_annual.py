...
#from edc.contants import DEAD

from apps.bcpp_survey.models import Survey
from apps.bcpp_household_member.constants import ABSENT, HTC_ELIGIBLE, HTC, BHS_SCREEN, ANNUAL, NOT_ELIGIBLE, UNDECIDED
...
class OperationalAnnual(BaseOperationalReport):
    ...
    def report_data(self):
        ...
        targeted_members_baseline = HouseholdMember.objects.filter(household_structure__household__plot__community__icontains=self.community,
        ...
        # Create new threads
        thread1 = AnnualMember(1, "AnnualMember-1", 1, previous_survey, targeted_members_internal_identifiers, self.data_dict,
        ...
        thread2 = TargetedMember(2, "TargetedMember-2", 2, previous_survey, current_annual_survey, targeted_members,
                  targeted_members_internal_identifiers, self.data_dict, self.community, self.date_from, self.date_to,
                  self.ra_username)
...
class AnnualMember(threading.Thread):
    ...
    def run(self):
        ...
        annual_new_hic_enrolled = HicEnrollment.objects.filter(subject_visit__household_member__household_structure__household__plot__community__icontains=self.community,
                                            ...
                                            hic_permission='Yes').exclude(subject_visit__household_member__internal_identifier__in=self.targeted_members_internal_identifiers).count()
...
class NewMember(threading.Thread):
    ...
    def run(self):
        print "Starting " + self.name
        new_members_aggregate = HouseholdMember.objects.filter(household_structure__household__plot__community__icontains=self.community,
