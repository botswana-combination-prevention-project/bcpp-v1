...
class OperationalPlots(BaseOperationalReport):
    ...
    def report_data(self):
        ...
        dispatch_container_reg = DispatchContainerRegister.objects.filter(is_dispatched=True)
        dispatched_identifiers = [container.container_identifier for container in dispatch_container_reg]
        ...
        self.data_dict['1. Total No. of plots'] = plt.filter(
        ...
        inaccessible_log_entry = PlotLogEntry.objects.filter(
            plot_log__plot__community__icontains=self.community,
            plot_log__plot__status=INACCESSIBLE,
            modified__gte=self.date_from, modified__lte=self.date_to,
            user_modified__icontains=self.ra_username,
            log_status=INACCESSIBLE)
        annotated_log_enry = inaccessible_log_entry.values(
            'plot_log__plot__plot_identifier').annotate(dcount=Count('plot_log__plot__plot_identifier'))
        ...
        in_accessible_plots = plt.filter(
            community__icontains=self.community,
            modified__gte=self.date_from, modified__lte=self.date_to,
            user_modified__icontains=self.ra_username,
        ...
        confirmed = plt.filter(
            action=CONFIRMED, community__icontains=self.community,
            modified__gte=self.date_from, modified__lte=self.date_to,
            user_modified__icontains=self.ra_username).count()
        self.data_dict['8. Plots confirmed'] = confirmed
        un_confirmed = plt.filter(
            action=UNCONFIRMED, community__icontains=self.community,
            modified__gte=self.date_from, modified__lte=self.date_to,
            user_modified__icontains=self.ra_username).count()
        self.data_dict['9. Plots not confirmed'] = un_confirmed
        plots_enmerated_structures = HouseholdStructure.objects.filter(
            household__plot__community__icontains=self.community,
            modified__gte=self.date_from, modified__lte=self.date_to,
            user_modified__icontains=self.ra_username,
            enumerated=True
        )
        plots_enumerated = plots_enmerated_structures.values(
            'household__plot__plot_identifier').annotate(dcount=Count('household__plot__plot_identifier'))
        self.data_dict['91. Enumerated plots'] = len(plots_enumerated)
        res_habitable = plt.filter(
            status=RESIDENTIAL_HABITABLE, community__icontains=self.community,
            modified__gte=self.date_from, modified__lte=self.date_to,
            user_modified__icontains=self.ra_username).count()
        self.data_dict['92. Residential habitable plots'] = res_habitable
        res_not_habitable = plt.filter(
            status=RESIDENTIAL_NOT_HABITABLE, community__icontains=self.community,
            modified__gte=self.date_from, modified__lte=self.date_to,
            user_modified__icontains=self.ra_username).count()
        self.data_dict['93. Residential not habitable plots'] = res_not_habitable
        non_residential = plt.filter(
            status=NON_RESIDENTIAL, community__icontains=self.community,
            modified__gte=self.date_from, modified__lte=self.date_to,
            user_modified__icontains=self.ra_username).count()
        self.data_dict['94. Not residential plots'] = non_residential
        enrolled_plots = plt.filter(
            bhs=True, community__icontains=self.community,
            modified__gte=self.date_from, modified__lte=self.date_to,
            user_modified__icontains=self.ra_username).count()
...
#         self.data_dict['12. Plots not reached'] = not_reached
        return collections.OrderedDict(sorted(self.data_dict.items()))
