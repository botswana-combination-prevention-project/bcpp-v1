...
class HivCareAdherenceForm (BaseSubjectModelForm):
    def clean(self):
        ...
        if cleaned_data.get('on_arv', None) == 'Yes' and (cleaned_data.get('arv_stop_date') or cleaned_data.get('arv_stop')):
            raise forms.ValidationError('Do not indicate arv STOP DATE or STOP REASON if subject is currently taking ARV\'s')
        ...
        if cleaned_data.get('ever_taken_arv', None) == 'Yes' and cleaned_data.get('why_no_arv'):
            raise forms.ValidationError('If participant has not taken ARV\'s in the past, DO NOT give the main reason why they have not started.')
