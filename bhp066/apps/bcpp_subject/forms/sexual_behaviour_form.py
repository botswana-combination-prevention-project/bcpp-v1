...
class SexualBehaviourForm (BaseSubjectModelForm):
    ...
    def clean(self):
        ...
        #if participant has had sex in their lifetime, lifetime partners has to be >0
        #Commented out (reason being that some people do not want to answer such questions???)
#         if cleaned_data.get('ever_sex') == 'Yes' and cleaned_data.get('lifetime_sex_partners') == 0:
#             raise forms.ValidationError('If participant has had sex in their lifetime, number of people he/she has had sex with should be greater than zero')
