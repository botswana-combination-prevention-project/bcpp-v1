...
class HivTestedForm (BaseSubjectModelForm):
    ...
    def clean(self):
        ...
        if cleaned_data.get('hiv_pills') == 'Yes' and not cleaned_data.get('arvs_hiv_test'):
            raise forms.ValidationError('if participant has heard about ARV\'s, provide information whether he/she believes that HIV positive can live longer if taking ARV\'s')
        # notsure
        if cleaned_data.get('hiv_pills') == 'not_sure' and not cleaned_data.get('arvs_hiv_test'):
            raise forms.ValidationError('Even if participant is not sure he/she heard about ARV\'s, provide information whether he/she believes that HIV positive can live longer if taking ARV\'s')
        return cleaned_data
