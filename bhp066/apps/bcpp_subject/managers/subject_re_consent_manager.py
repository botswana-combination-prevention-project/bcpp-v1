...
class SubjectReConsentManager(models.Manager):
    ...
    def get_queryset(self):
        if settings.LIMIT_EDIT_TO_CURRENT_COMMUNITY:
            ...
            if PlotIdentifier.get_notebook_plot_lists():
                return super(
                    SubjectReConsentManager, self).get_queryset().filter(
                        community=community,
                        household_member__household_structure__household__plot__plot_identifier__in=PlotIdentifier.get_notebook_plot_lists())
            else:
                return super(SubjectReConsentManager, self).get_queryset().filter(community=community)
        return super(SubjectReConsentManager, self).get_queryset()
