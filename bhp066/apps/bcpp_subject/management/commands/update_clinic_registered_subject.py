...
class Command(BaseCommand):
    ...
    def determine_bcpp_registered_subject(self, identity):
        ...
        hhm = HouseholdMember.objects.filter(
            Q(registered_subject__identity='565616307'),
    ...
    def get_all_clinic_models(self):
        clinic_models = [
            ClinicConsent, Appointment, ScheduledEntryMetaData, ClinicOffStudy #ClinicHouseholdMember 
        ]
    ...
    def update_clinic_appointment(self, bcpp_registered_subject, clinic_model):
            ...
            try:
                mymodel = clinic_model.objects.get(
                    registered_subject__identity=bcpp_registered_subject.identity, visit_definition__code='C0'
                                                   )
    ...
    def update_scheduledentrymetadata(self, bcpp_registered_subject, clinic_model):
        ...
        try:
            mymodel = clinic_model.objects.get(
                registered_subject__identity=bcpp_registered_subject.identity, appointment__visit_definition__code='C0')
            mymodel.registered_subject = bcpp_registered_subject
            mymodel.save()
        except clinic_model.DoesNotExist:
            pass
    ...
    def update_clinic_consent(self, bcpp_registered_subject, clinic_model):
        try:
            try:
                mymodel = clinic_model.objects.get(
                    Q(househohold_member__household_structure__household__plot__status='bcpp_clinic'),
                    Q(identity=bcpp_registered_subject.identity))
                mymodel.registered_subject = bcpp_registered_subject
                mymodel.save()
    ...
    def replace_registered_subject_for_clinic_models_audit(self, bcpp_registered_subject, clinic_model):
        try:
            mymodel = None
            try:
                mymodel = clinic_model.history.get(registered_subject__identity=bcpp_registered_subject.identity)
                mymodel.registered_subject = bcpp_registered_subject
                mymodel.save()
            ...
            print ('multiple on {} - {}'.format(clinic_model._meta.model_name, bcpp_registered_subject))
            if clinic_model._meta.model_name == 'appointment':
                mymodel = None
                try:
