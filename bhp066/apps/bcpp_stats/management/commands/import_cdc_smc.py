        ...
        with open(source_path, 'r') as f:
            ...
            for i, row in enumerate(rows):
                ...
                if not header_row:
                    ...
                    if len(row[header_row.index('mcVstIDtypeOM')]) == 9 and row[header_row.index('mcVstIDtypeOM')][4] in ['1', '2']:
                        pass
                    elif len(row[header_row.index('mcVstIDtypeOM')]) == 8 and row[header_row.index('mcVstIDtypeOM')][3] in ['1', '2']:
                        ...
                        print 'Row {0}. Invalid OMANG in field {1}. Got {2}'.format(i, 'mcVstIDtypeOM', row[header_row.index('mcVstIDtypeOM')] or 'None')
                    ...
                    if dob and reference_date:
                        ...
                        if age_in_years != int(cdc_age_in_years):
                            print 'Row {0}. Calculated age_in_years does not match mcVstInfoAge by {1}! Got {2}<>{3}'.format(i, age_in_years - int(cdc_age_in_years), age_in_years, cdc_age_in_years)
                        if age_in_years < 16:
