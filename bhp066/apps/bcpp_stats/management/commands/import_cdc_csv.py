...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.empty_fields = []  # fields in the model but not in the header row
        ...
        with open(source_path, 'r') as file_object:
            ...
            for i, row in enumerate(rows):
                if not header_row:
                    ...
                    # confirm that the row is correctly split, e.g. formatted correctly without CR, LF etcim
                    self.import_cdc_data(
                        i, row, header_row, model_cls, system_fields, additional_fields, model_name, source_path)
        ...
        self.empty_fields = list(set(self.empty_fields))
        self.empty_fields.sort()
    ...
    def date_field(self, row_index, row, header_row, field, model_name):
        try:
                    ...
                    try:
                        ...
                        raise ValueError(
                            ...
                            'to DateTime. Got {2}. {3}.'.format(
                                row_index, field.name, x or None, error_msg, model_name))

    def integer_field(self, row_index, row, header_row, field, model_name):
        try:
    ...
    def prep_import_data(self, row_index, row, field, header_row, system_fields, additional_fields, model_name):
            ...
            try:
                ...
                if not row_val or row[header_row.index(field.name)] == '.':
                    ...
                    if 'Date' in field.get_internal_type():
                        ...
                        self.date_field(row_index, row, header_row, field, model_name)
                    elif 'Integer' in field.get_internal_type():
                        # model field is an IntegerField
                        self.integer_field(row_index, row, header_row, model_name)
                        ...
                        self.float_field(self, row, field, header_row, model_name)
                    else:
    ...
    def import_cdc_data(self, index, row, header_row, model_cls, sys_fields, addtnl_fields, model_name, source_path):
        ...
        for field in model_cls._meta.fields:
            ...
            if field.name not in header_row + sys_fields + addtnl_fields:
                self.empty_fields.append(field.name)
            else:
                ...
                self.prep_import_data(
                    index, row, field, header_row, sys_fields, addtnl_fields, model_name)
    ...
    def float_field(self, row, field, header_row, model_name):
        try:
            ...
            # conversion of cell value to integer failed
            x = row[header_row.index(field.name)]
            raise ValueError('in {3}, {0}. (value={2}) {1}'.format(field.name, error_mgs, x, model_name))
