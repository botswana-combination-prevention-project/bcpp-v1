    ...
    def __init__(self):
        ...
        for community in self.community_list:
            with open(os.path.join(
                    os.path.expanduser('~/export_to_cdc'),
                    'Enrolled{}-{}.csv'.format(community.title(), datetime.today().strftime('%Y%m%d%H%M%s'))),
                    'a'
            ) as f:
                writer = csv.writer(f)
                ...
                for hic_enrollment in HicEnrollment.objects.filter(
                    ...
                    writer.writerow([
                        community,
                        hic_enrollment.subject_visit.household_member.registered_subject.subject_identifier, 'HIC',
