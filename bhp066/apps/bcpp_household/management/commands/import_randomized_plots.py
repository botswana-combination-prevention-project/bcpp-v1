        ...
        if not (os.path.exists(file_75_list) and os.path.exists(file_20_list) and os.path.exists(file_5_list)):
            ...
            for plot in lines_20:
                ...
                if Plot.objects.filter(plot_identifier=plot[0]):
                    ...
                    lines_20 = Plot(plot_identifier=plot[0], gps_target_lat=plot[1], gps_target_lon=plot[2], selected=1, community=community_name)
                    lines_20.save()
                num_20_pct += 1
            ...
            for plot in lines_5:
                ...
                if Plot.objects.filter(plot_identifier=plot[0]):
                    ...
                    lines_5 = Plot(plot_identifier=plot[0], gps_target_lat=plot[1], gps_target_lon=plot[2], selected=2, community=community_name)
                    lines_5.save()
                num_5_pct += 1
