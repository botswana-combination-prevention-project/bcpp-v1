...
class HouseholdIdentifier(BaseIdentifier):
    ...
    def household_sequence(self):
        ...
        ident_historys = self.get_identifier_history_model().objects.filter(
            plot_identifier=self.plot_identifier)
        sequences_in_use = [
            ident_history.identifier.split(self.plot_identifier)[1].split('-')[0] for ident_history in ident_historys]
