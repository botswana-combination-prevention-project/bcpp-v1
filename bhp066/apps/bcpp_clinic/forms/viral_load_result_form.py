...
class ViralLoadResultForm (BaseModelForm):
    ...
    def clean(self):
        ...
        vl_requisition_metadata = RequisitionMetaData.objects.get(appointment=cleaned_data.get('clinic_visit').appointment, lab_entry=LabEntry.objects.get(requisition_panel__name='Viral Load'))
        if vl_requisition_metadata.entry_status == NEW:
            raise forms.ValidationError('If a Viral Load Requisition is required, then you have to fill it before this form.')
