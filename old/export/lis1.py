...
class Lis(object):
    ...
    For example:
        ...
        lis = Lis('BHP066', subject_type='clinic')
        lis.results
    ...
    def __init__(self, protocol, df=pd.DataFrame(), subject_type=None, engine=None, protocol_prefix=None):
        ...
        if not df.empty:
            ...
            if self.subject_type == 'clinic':
                self.requisition = Requisition(ClinicRequisition, subject_type=self.subject_type).dataframe
            elif self.subject_type == 'subject':
                self.requisition = Requisition(SubjectRequisition, subject_type=self.subject_type).dataframe
