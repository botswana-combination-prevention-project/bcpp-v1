...
from edc_core.bhp_common.choices import YES_NO, YES_NO_UNSURE
from edc_core.audit_trail.audit import AuditTrail
from edc_core.bhp_adverse.models import BaseBaseDeath


class SubjectDeath(BaseBaseDeath):

    sufficient_records = models.CharField(
        verbose_name="Are sufficient records for the 30 days before death available for review? ",
        max_length=3,
        choices=YES_NO,
    ...
    document_hiv = models.CharField(
        verbose_name="Does the record document that the decedent had HIV? ",
        max_length=3,
        choices=YES_NO,
        validators=[eligible_if_yes, ],
        help_text=" if 'NO' STOP, not eligibile",
    ...
    document_community = models.CharField(
        verbose_name="Does the record document that the decedent lived one of the study communities? ",
        max_length=3,
        choices=YES_NO,
        validators=[eligible_if_yes, ],
        help_text=" if 'NO' STOP, not eligibile",
    ...
    death_year = models.DateField(
        verbose_name="What was the month and year of death?  ",
        help_text="if 'More than 6 months prior,' STOP, not eligible.",
        )
    decendent_death_age = models.IntegerField(
        verbose_name="What was the age in years of the decedent at the time of death? ",
        max_length=2,
    ...
    hospital_death = models.CharField(
        verbose_name="Did the decedent die in the hospital? ",
        max_length=3,
        choices=YES_NO,
        help_text="",
        )
    decedent_haart = models.CharField(
        verbose_name=("Was the decedent receiving antiretroviral therapy (HAART) during the 30 days"
    ...
    decedent_hospitalized = models.CharField(
        verbose_name="Did resident spend at least one night in the hospital within 30 days of his/her death?",
        max_length=3,
        choices=YES_NO,
    ...
    hospital_visits = models.IntegerField(
        verbose_name=("How many visits did the decedent make to the health post, clinic,"
