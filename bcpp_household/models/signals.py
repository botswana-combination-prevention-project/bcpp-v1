...
@receiver(post_save, weak=False, dispatch_uid="post_save_create_household")
def post_save_create_household(sender, instance, **kwargs):
    if not kwargs.get('raw', False):
        if isinstance(instance, Plot):
            instance.post_save_create_household(**kwargs)