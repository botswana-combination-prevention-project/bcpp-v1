...
class DetailedSexualHistoryPage(BaseModelAdminPage):
    ...
    def set_report_date(self, report_date):
        date_element = self.browser.find_element(*DetailedSexualHistoryPage.report_date)
        date_element.send_keys(report_date)

    def set_report_time(self, report_time):
        time_element = self.browser.find_element(*DetailedSexualHistoryPage.report_time)
        time_element.send_keys(report_time)

    def set_first_partner_live(self, first_partner_live):
        first_partner_live_element = self.browser.find_element(*DetailedSexualHistoryPage.first_partner_live)
        first_partner_live_element.send_keys(first_partner_live)

    def set_sex_partner_community(self, sex_partner_community):
        sex_partner_community_element = self.browser.find_element(*DetailedSexualHistoryPage.sex_partner_community)
        sex_partner_community_element.send_keys(sex_partner_community)

    def set_past_year_sex_freq(self, past_year_sex_freq):
        past_year_sex_freq_element = self.browser.find_element(*DetailedSexualHistoryPage.past_year_sex_freq)
        past_year_sex_freq_element.send_keys(past_year_sex_freq)

    def set_third_last_sex(self, third_last_sex):
        third_last_sex_element = self.browser.find_element(*DetailedSexualHistoryPage.third_last_sex)
        third_last_sex_element.send_keys(third_last_sex)

    def set_first_first_sex(self, first_first_sex):
        first_first_sex_element = self.browser.find_element(*DetailedSexualHistoryPage.first_first_sex)
        first_first_sex_element.send_keys(first_first_sex)

    def set_first_sex_current(self, first_sex_current):
        first_sex_current_element = self.browser.find_element(*DetailedSexualHistoryPage.first_sex_current)
        first_sex_current_element.send_keys(first_sex_current)

    def set_first_relationship(self, first_relationship):
        first_relationship_element = self.browser.find_element(*DetailedSexualHistoryPage.first_relationship)
        first_relationship_element.send_keys(first_relationship)

    def set_first_exchange(self, first_exchange):
        first_exchange_element = self.browser.find_element(*DetailedSexualHistoryPage.first_exchange)
        first_exchange_element.send_keys(first_exchange)

    def set_concurrent(self, concurrent):
        concurrent_element = self.browser.find_element(*DetailedSexualHistoryPage.concurrent)
        concurrent_element.send_keys(concurrent)

    def set_goods_exchange(self, goods_exchange):
        goods_exchange_element = self.browser.find_element(*DetailedSexualHistoryPage.goods_exchange)
        goods_exchange_element.send_keys(goods_exchange)

    def set_first_sex_freq(self, first_sex_freq):
        first_sex_freq_element = self.browser.find_element(*DetailedSexualHistoryPage.first_sex_freq)
        first_sex_freq_element.send_keys(first_sex_freq)

    def set_first_partner_hiv(self, first_partner_hiv):
        first_partner_hiv_element = self.browser.find_element(*DetailedSexualHistoryPage.first_partner_hiv)
        first_partner_hiv_element.send_keys(first_partner_hiv)

    def set_partner_hiv_test(self, partner_hiv_test):
        partner_hiv_test_element = self.browser.find_element(*DetailedSexualHistoryPage.partner_hiv_test)
        partner_hiv_test_element.send_keys(partner_hiv_test)

    def set_first_disclose(self, first_disclose):
        first_disclose_element = self.browser.find_element(*DetailedSexualHistoryPage.first_disclose)
        first_disclose_element.send_keys(first_disclose)

    def set_first_condom_freq(self, first_condom_freq):
        first_condom_freq_element = self.browser.find_element(*DetailedSexualHistoryPage.first_condom_freq)
        first_condom_freq_element.send_keys(first_condom_freq)

    def set_first_partner_cp(self, first_partner_cp):
        first_partner_cp_element = self.browser.find_element(*DetailedSexualHistoryPage.first_partner_cp)
