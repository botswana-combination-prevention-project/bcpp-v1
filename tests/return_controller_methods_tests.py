...
from bhp_dispatch.models import TestItem, DispatchItemRegister, DispatchContainerRegister
...
class ReturnControllerMethodsTests(BaseControllerTests):
    ...
    def test_return_controller(self):
        ...
        self.base_dispatch_controller.dispatch_as_json(obj, dispatch_container)
        ...
        self.assertEqual(DispatchItemRegister.objects.filter(is_dispatched=False, return_datetime__isnull=False).count(), 1)
        ...
        self.assertEqual(DispatchItemRegister.objects.filter(is_dispatched=False).count(), 1)
