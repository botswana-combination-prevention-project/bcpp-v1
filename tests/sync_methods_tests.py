...
class SyncMethodsTests(unittest.TestCase):
    ...
    def test_fetch_from_producer(self):
        ...
        self.get_consumer_intance().fetch_from_producer(self.producer_name)
    ...
    def test_get_producer(self):
        ...
        producer = self.get_consumer_intance().get_producer(self.producer_name)     
    ...
    def test_transactions_sync_deserialize(self):
        ...
        #delete one record in the producer
        for k in ConsentCatalogue.objects.using(self.using_destination).all():
            k.delete(using=self.using_destination)
        #check for one more outgoing transaction in the producer
        self.assertEqual(OutgoingTransaction.objects.using(self.using_destination).all().count(), 6)
        self.create_producer()
        #sync outgoing transactions from the producer
        self.get_consumer_intance().fetch_from_producer(self.using_destination)
        #make sure all transactions were consumed from the producer
        self.assertFalse(OutgoingTransaction.objects.using(self.using_destination).filter(is_consumed=False,
                                                                                          ).exists())
        ...
        self.assertFalse(IncomingTransaction.objects.using(self.using_destination).filter(is_consumed=False,
                                                                                          ).exists())
