...
class BaseMembershipForm(BaseModelForm):
    ...
    def clean(self):
        ...
        if cleaned_data:
            ...
            subject_consent_model_cls = SubjectConsent.objects.get(household_structure_member.survey.survey_slug)
            subject_consent = subject_consent_model_cls.objects.filter(household_structure_member=household_structure_member)
            if subject_consent:
                subject_consent = SubjectConsent.objects.get(household_structure_member=household_structure_member)
