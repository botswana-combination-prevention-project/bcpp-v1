...
class Infant(object):
    ...
    def get_identifier(self, user, **kwargs):
        ...
        *Information comes from a deliver form usually, no a cosent. 
        *Names, initials, gender and DOB are not expected to be known yet.
        *Subject Identifier is derived from the maternal identifier so the
        SubjectIdentifier model is not updated."""
        ...
        for infant_order in range(0, live_infants_to_register):
            ...
            DerivedSubjectIdentifier.objects.create(subject_identifier=subject_identifier['infant_order'], 
                                                    base_identifier=maternal_identifier)
        #return a dictionary { infant order: identifier }    
        return subject_identifier
        #return self._register_infants(user, maternal_identifier, maternal_site, subject_type, live_infants, live_infants_to_register)
            
    def _register_infants(self, user, maternal_identifier, maternal_site, subject_type, live_infants, live_infants_to_register):
        ...
        for infant_order in range(0, live_infants_to_register):
            ...
            # Names, initials, gender and DOB are not expected to be known yet
            self.RegisteredSubject.objects.create(    
