...
class Identifier(object):
    ...
    Create or increment identifier base36 encoded based on a given site_code and the year.
        ...
        Note that the check-digit is part of the encoded number but not the decoded_number
    
        If a decoded number is passed without a check digit, it will be added before encoding but
        remove after decoding.
        
        If a number is passed to the encoder with a check_digit, it will be remove upon decoding.
    ...
    def create_with_root(self, given_root_segment, counter_length=0):
        ...
        """Create a new system-wide unique identifier with a given root segment with or without a counter segment depending on the value of counter_length."""   
