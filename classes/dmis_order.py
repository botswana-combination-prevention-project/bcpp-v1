...
class DmisOrder(object):
    ...
    def fetch(self, receive=None, **kwargs):
        ...
        for receive in receives:
            logger.info('  dmis - %s %s/%s' % (receive.receive_identifier, receive_count, tot))
            receive_count -= 1
    ...
    def create_or_update_order(self, row, **kwargs):

        orders = Order.objects.using(self.lab_db).filter(order_identifier=row.order_identifier)
        if orders:
            ...
            logger.info('  dmis - order: created {order_identifier}'.format(order_identifier=row.order_identifier))
        return order
    ...
    def fetch_or_create_panel(self, **kwargs):
        ...
        if not panel and tid:
            panels = Panel.objects.using(self.lab_db).filter(dmis_panel_identifier__exact=tid)
            if panels:
                panel = panels[0]
    ...
    def fetch_or_create_aliquot(self, **kwargs):
        ...
        if not create:
            ...
            if aliquot:
                ...
                if self.debug:
                    logger.info('  created aliquot from %s ' % (primary_aliquot.aliquot_identifier,))
