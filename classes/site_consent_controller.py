...
class SiteConsentController(object):
    ...
    def register(self, consent_cls, consent_update_cls):
        ...
        # confirm consent is in Consent Catalogue
        #if not ConsentCatalogue.objects.filter(content_type_map__model=consent_cls._meta.object_name.lower()):
        #    raise AttributeError('Unable to register consent model {0}. Consent models must be listed in the Consent Catalogue.'.format(consent_cls._meta.object_name))
