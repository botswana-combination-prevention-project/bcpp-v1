...
class BaseKeyToConsentModelAdmin(BaseModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        ...
        return super(BaseKeyToConsentModelAdmin, self).save_model(request, obj, form, change)
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        super(BaseKeyToConsentModelAdmin, self).get_readonly_fields(request, obj)
        consent_fk_name = [fk for fk in [f for f in self.model._meta.fields if isinstance(f, ForeignKey)] if fk.rel.to._meta.module_name == self.consent_model._meta.module_name][0].name
        if obj:  # In edit mode
