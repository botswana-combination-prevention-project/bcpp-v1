...
#         self.include_after_exclusion_model_keyed = ['subjectreferral', ]
    ...
    def create(self, **kwargs):
        ...
        # must have this
        self.household_member = kwargs.get('household_member')
        ...
        if self.household_member.household_structure:
            ...
            household_members = HouseholdMember.objects.filter(
                                              household_structure_id=self.household_member.household_structure_id
        ...
        if not self.subject_identifier:
            # subject is not consented for this survey, just a household_member
            ...
            # for any survey
            if RegisteredSubject.objects.filter(registration_identifier=self.household_member.internal_identifier).exists():
                registered_subject = RegisteredSubject.objects.get(registration_identifier=self.household_member.internal_identifier)
                if registered_subject.subject_identifier:
                    ...
                    if self.household_member.household_structure:
                        self.dashboard_identifier = '%s (%s) %s of household %s' % (registered_subject.first_name,
                                                                                    ...
                                                                                    self.household_member.household_structure.household.household_identifier)
                    else:
                ...
                # created when the household_member
                # instance is created.
                self.dashboard_identifier = '%s (%s) %s NEW MEMBER' % (self.household_member.first_name,
                                                                       self.household_member.initials,
                                                                       self.household_member.gender)
        ...
        kwargs.update({'household_members': household_members})
        kwargs.update({'household_structure': self.household_structure})
        kwargs.update({'extra_url_context': '&household_member=%s&survey=%s' % (self.household_member.pk, self.survey.survey_slug,)})
