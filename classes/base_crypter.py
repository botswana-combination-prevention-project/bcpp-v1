...
class BaseCrypter(Base):
        ...
        def load_key(algorithm, mode=None, key_name=None):
            ...
            if algorithm == 'rsa':
                if key_name == 'public':
                    ...
                    self.set_encrypted_salt(algorithm=algorithm, mode=mode)
                    key = self.encrypted_salt
                    self.encrypted_salt = None
                else:
    ...
    def set_encrypted_salt(self, **kwargs):
        """ Sets and returns the encrypted_salt for the given algorithm and mode.

        Gets from IS_PRELOADED or reads from the file but will always reset it."""
        algorithm = kwargs.get('algorithm', None)
        mode = kwargs.get('mode', None)
        if (self.PRELOADED_KEYS.get(algorithm).get(mode).get('salt') and
            ...
            self.encrypted_salt = self._read_encrypted_salt_from_file(algorithm=algorithm, mode=mode)
