...
class ModelAdminUrl(template.Node):
    ...
    def __init__(self, contenttype, visit_model, appointment, dashboard, app_label):
    ...
    def render(self, context):
        ...
        if self.contenttype.model_class().objects.filter(visit = self.visit_model.pk):
            ...
            model = self.contenttype.model_class().objects.get(visit = self.visit_model.pk)
            ...
            rev_url = '%s?next=%s&dashboard=%s&subject_identifier=%s&visit_code=%s&visit_instance=%s' % (
                                                                       rev_url, next, self.dashboard, 
                                                                       self.visit_model.appointment.registered_subject.subject_identifier, 
                                                                       self.visit_model.appointment.visit_definition.code, 
                                                                       self.visit_model.appointment.visit_instance )            
        else:
            ...
            try:
                ...
                rev_url = '%s?visit=%s&next=%s&dashboard=%s&subject_identifier=%s&visit_code=%s&visit_instance=%s' % (
                                                                        rev_url, 
                                                                        self.visit_model.pk, next, self.dashboard, 
                                                                        self.visit_model.appointment.registered_subject.subject_identifier, 
                                                                        self.visit_model.appointment.visit_definition.code, 
                                                                        self.visit_model.appointment.visit_instance)            
            except:
...
def model_admin_url(parser, token):
    ...
    try:
        tag_name, contenttype, visit_model, appointment, dashboard, app_label = token.split_contents()
    except ValueError:
        raise template.TemplateSyntaxError("%r tag requires exactly 5 arguments" % token.contents.split()[0])
    return ModelAdminUrl(contenttype, visit_model, appointment, dashboard, app_label)
