...
class ModelPk(template.Node):
    ...
    def render(self, context):
        ...
        fk_fieldname_to_visit_model = visit_model_helper.get_visit_field(model=this_model, visit_model=self.visit_model)
    ...
    def render(self, context):
        ...
        if not fk_fieldname_to_visit_model:
            raise AttributeError('Cannot reverse for model, Model %s must have a foreignkey to the visit model \'%s\'. '
                                 ...
                                 'bhp_visit.Entry.' % (this_model._meta.object_name,
