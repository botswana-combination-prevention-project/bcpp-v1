...
class GetRequisitionIdentifier(template.Node):

    """return a requisition identifier"""

    def __init__(self, requisition_model, panel, registered_subject, appointment, visit_model):
        self.unresolved_requisition_model = template.Variable(requisition_model)
        self.unresolved_panel = template.Variable(panel)        
        self.unresolved_registered_subject = template.Variable(registered_subject)
        self.unresolved_appointment = template.Variable(appointment)                
    ...
    def render(self, context):
        ...
        if req:        
            return req[0].requisition_identifier 
        else:
            return 'not drawn'

@register.tag(name='get_requisition_identifier')
def get_requisition_identifier(parser, token):

    """Compilation function for renderer GetRequisitionIdentifier """ 

    try:
        tag_name, requisition_model, panel, registered_subject, appointment, visit_model = token.split_contents()
    except ValueError:
        raise template.TemplateSyntaxError("%r tag requires exactly 5 arguments" % token.contents.split()[0])
    return GetRequisitionIdentifier(requisition_model, panel, registered_subject, appointment, visit_model)
