...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        field_crypter = FieldCrypter('aes', 'local')
        model = None
        if not args:
            self.stdout.write('Encrypting both incoming and outgoing.\n')
            self.stdout.write('Starting with incoming.\n')
            self.encrypt_tx(field_crypter, get_model('bhp_sync', 'incomingtransaction'))

            self.stdout.write('Processing with outgoing.\n')
            self.encrypt_tx(field_crypter, get_model('bhp_sync', 'outgoingtransaction'))
        else:
            ...
            if model:
                self.encrypt_tx(field_crypter, model)

    def encrypt_tx(self, field_crypter, model):
        ...
        for transaction in model.objects.exclude(tx__startswith='enc1:::'):
            ...
            transaction.tx = field_crypter.encrypt(transaction.tx)
            transaction.save()
